...
void QextSerialEnumerator::setUpNotifications( )
{
    ...
    if (KERN_SUCCESS != kernResult) {
        qDebug() << "[QEXTSERIALENUMERATOR] IOMasterPort returned:" << kernResult;
        return;
    }

    classesToMatch = IOServiceMatching(kIOSerialBSDServiceValue);
    if (classesToMatch == NULL)
        qDebug("[QEXTSERIALENUMERATOR] IOServiceMatching returned a NULL dictionary.");
    else
        CFDictionarySetValue(classesToMatch, CFSTR(kIOSerialBSDTypeKey), CFSTR(kIOSerialBSDAllTypes));

    if( !(cdcClassesToMatch = IOServiceNameMatching("AppleUSBCDC") ) ) {
        qWarning("[QEXTSERIALENUMERATOR] couldn't create cdc matching dict");
        return;
    }
    ...
    if(notificationPortRef == NULL) {
        qDebug("[QEXTSERIALENUMERATOR] IONotificationPortCreate return a NULL IONotificationPortRef.");
        return;
    }

    notificationRunLoopSource = IONotificationPortGetRunLoopSource(notificationPortRef);
    if (notificationRunLoopSource == NULL) {
        qDebug("[QEXTSERIALENUMERATOR] IONotificationPortGetRunLoopSource returned NULL CFRunLoopSourceRef.");
        return;
    }
